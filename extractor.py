...
def extract_table_content_markdown(input_path, clean_pdf_output=""):
    global converter
    ...
    with redirect_stdout(log_capture), redirect_stderr(log_capture):
        ...
        try:
            ...
            if clean_pdf_output:
                ...
                clean_dir = os.path.join(input_dir, "clean_pdfs")
...
def extract_table_content_csv(input_path, clean_pdf_output=""):
    global converter
    log_capture = io.StringIO()
    table_csvs = []
    ...
    with redirect_stdout(log_capture), redirect_stderr(log_capture):
        ...
        root_logger.addHandler(temp_handler)
        try:
            # 1. Extract input directory and base filename
            input_dir = os.path.dirname(input_path)
            filename = os.path.basename(input_path)
            
            # 2. Resolve the clean path dynamically based on clean_pdf_output
            if clean_pdf_output:
                if clean_pdf_output.lower().endswith('.pdf'):
                    clean_path = clean_pdf_output
                else:
            ...
            for table in parsed_result.document.tables:
                table_df = table.export_to_dataframe(doc=parsed_result.document)
                csv_str = table_df.to_csv()
                table_csvs.append(csv_str)
        finally:
            root_logger.removeHandler(temp_handler)
            
    return table_csvs, log_capture.getvalue()
